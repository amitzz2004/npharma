...
import {
  FaUser, FaBuilding, FaPhoneAlt, FaEnvelope, FaMapMarkerAlt,
  FaPills, FaBoxes, FaClock, FaFileInvoice,
} from "react-icons/fa";
import Button from "../common/Button";
import { useFetch } from "../../hooks/useFetch";
import { useApp } from "../../context/AppContext";
import { submitQuotationForm } from "../../services/api";
...
const QuotationForm = () => {
  ...
  const { execute, loading } = useFetch(submitQuotationForm);
  ...
  const validate = () => {
    ...
    if (!formData.productDetails.trim()) errors.productDetails = "Please describe the products you need";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast("success", res?.message || "Quotation request sent successfully!");
    ...
    <form className="quotation-form" onSubmit={handleSubmit} noValidate>
      <div className="form-row">
        <div className="form-group">
          ...
          <input type="text" name="customerName" placeholder="Your full name" value={formData.customerName} onChange={handleChange} />
          {fieldErrors.customerName && <span className="field-error">{fieldErrors.customerName}</span>}
        </div>
        <div className="form-group">
          <label><FaBuilding /> Company Name</label>
          <input type="text" name="companyName" placeholder="Business / Hospital name" value={formData.companyName} onChange={handleChange} />
        </div>
      </div>
      ...
      <div className="form-row">
        <div className="form-group">
          ...
          <input type="tel" name="phoneNumber" placeholder="+91 98765 43210" value={formData.phoneNumber} onChange={handleChange} />
      ...
      <div className="form-row">
        <div className="form-group">
          ...
          <input type="text" name="location" placeholder="City, State" value={formData.location} onChange={handleChange} />
      ...
      <div className="form-group">
        <label><FaBoxes /> Product Details *</label>
        <textarea
          name="productDetails"
          rows="4"
          placeholder="List the medicines / surgical items you need a quotation for"
          value={formData.productDetails}
          onChange={handleChange}
        />
        {fieldErrors.productDetails && <span className="field-error">{fieldErrors.productDetails}</span>}
      </div>

      <div className="form-row">
        <div className="form-group">
          <label><FaBoxes /> Quantity</label>
          <input type="text" name="quantity" placeholder="e.g. 500 units" value={formData.quantity} onChange={handleChange} />
        </div>
        <div className="form-group">
          <label><FaClock /> Timeline *</label>
          <input type="text" name="timeline" placeholder="e.g. Within a week" value={formData.timeline} onChange={handleChange} />
      ...
      <div className="form-group">
        ...
        <input type="text" name="gstNumber" placeholder="e.g. 06ABCDE1234F1Z5" value={formData.gstNumber} onChange={handleChange} />
      </div>
